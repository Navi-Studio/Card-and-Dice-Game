...
namespace Map
{
    public class MapView : MonoBehaviour
    {
        ...
        [Header("Player")]
        [Tooltip("If the player sprite is null, player will not be shown")]
        //public Sprite player;
        public GameObject playerPrefab;
        //public GameObject playerObject;
        public float playerOffset = 0;
        public NodeBlueprint unknownBlueprints;
        ...
        protected virtual void CreateMapPlayer()
        {
            if (playerPrefab == null) return;

            // playerObject = Instantiate(playerPrefab, mapParent.transform);
            playerObject = playerPrefab;
            playerObject.transform.SetParent(mapParent.transform);
            var currentPosition = new Vector3(0f, 0f, 1f);
            ...
            //}
            playerObject.transform.localPosition = currentPosition;
            playerObject.transform.localRotation = Quaternion.identity;
            playerObject.transform.rotation = Quaternion.identity;
            playerObject.transform.localScale = new Vector3(0.1f, 0.1f, 1f);
        }
        ...
        protected virtual MapNode CreateMapNode(Node node)
        {
            ...
            mapNode.MaskBlurprint = unknownBlueprints;
            mapNode.SetUp(node, blueprint);
            mapNode.transform.localPosition = node.position;
            return mapNode;
        }

        public void SetPlayerPosition(MapNode mapNode)
        {
            if (mapNode == null) return;
            playerObject.transform.DOMove(mapNode.transform.position + new Vector3(playerOffset, 0, 0), 1f);
        }
        public void SetPlayerPosition(Transform mapNode)
        {
            if (mapNode == null) return;
            playerObject.transform.position =mapNode.position;
        }
        ...
        public void SetAttainableNodes()
        {
            ...
            else
            {
                ...
                foreach (var point in mapManager.CurrentMap.path)
                {
                    foreach (var node in MapNodes.Where(n => n.Node.point.y == point.y))
                ...
                foreach (var point in currentNode.outgoing)
                {
                    ...
                    if (mapNode != null)
                        mapNode.SetState(NodeStates.Attainable);
                    //int times = PlayerData.Instance.ATK;
                    //foreach (var pointp in mapManager.CurrentMap.GetNode(point).outgoing)
                    //{
                    //    var mapNodep = GetNode(pointp);
                    //    if (mapNodep != null)
                    //        mapNodep.SetState(NodeStates.Attainable);
                    //}
                }
            }
        ...
        private void DrawLines()
        {
            foreach (var node in MapNodes)
            {
                foreach (var connection in node.Node.outgoing)
                    AddLineConnection(node, GetNode(connection));   // 根据数对GetNode，要改成根据ID可以支持图结构
            }
        }
        ...
        {
            // all map configs = defaultmapconfig, angelDice
